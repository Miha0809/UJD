...
def _print_help():
    ...
    print("update <id> <name> - adds user to the database or updates its current information")
...
def run_command(db: StudentDatabase, cmd: str) -> bool:
    ...
    if cmd_splits[0] == "add":
        if len(cmd_splits) != 3:
            return False
        db.add(Student(int(cmd_splits[1]), cmd_splits[2]))
    elif cmd_splits[0] == "remove":
        # TODO: maybe here we have to check if it has two elements??
        db.removeById(int(cmd_splits[1]))
    elif cmd_splits[0] == "list":
        print(db.list())
    elif cmd_splits[0] == "update":
        if len(cmd_splits) != 3:
            return False
        db.update(Student(int(cmd_splits[1]), cmd_splits[2]))
    else:
        return False
