...
def genetic_algorithm(population_size, chromosome_length, generations, mutation_prob):
    ...
    for generation in range(generations):
        ...
        best_value = int(best_chromosome, 2)
        print(f"Generation {
              generation + 1}: Best = {best_value}, f(x) = {fitness_function(best_chromosome)}")

    best_chromosome = max(population, key=fitness_function)
    best_value = int(best_chromosome, 2)
    print(f"Best solution: x = {best_value}, f(x) = {
          fitness_function(best_chromosome)}")
